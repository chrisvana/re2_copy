...
// Prefilter is the class used to extract std::string guards from regexps.
...
class Prefilter {
 ...
 public:
  enum Op {
    ...
    ATOM,  // The std::string atom() must match
  ...
  // Set the children std::vector. Prefilter takes ownership of subs and
  ...
  // Returns a readable debug std::string of the prefilter.
 ...
 private:
  ...
  // Actual std::string to match in leaf node.
  std::string atom_;

  // If different prefilters have the same std::string atom, or if they are
  // structurally the same (e.g., OR of same atom std::strings) they are
