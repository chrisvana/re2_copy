...
enum InstOp {
  ...
  kInstEmptyWidth,   // empty-width special (^ $ ...); bit(s) std::vector in empty_
 ...
 public:
  ...
  class Inst {
   public:
    ...
    // Returns std::string representation for debugging.
    std::string Dump();
  ...
  enum MatchKind {
    ...
    kManyMatch       // for SearchDFA, records std::vector of matches
  };
  ...
  // Returns std::string representation of program for debugging.
  std::string Dump();
  ...
  // Returns the std::vector of kEmpty flags that are in effect at
  ...
  // match[0] is overall match, match[1] is first std::vector of parens, etc.
  // If a particular submatch is not matched during the regexp match,
  // it is std::vector to NULL.
  //
  // Matching text == StringPiece(NULL, 0) is treated as any other empty
  // std::string, but note that on return, it will not be possible to distinguish
  // submatches that matched that empty std::string from submatches that didn't
  ...
  // If the DFA runs out of memory, std::vectors *failed to true and returns false.
  ...
  // its own Match instruction recording the index in the std::vector.
