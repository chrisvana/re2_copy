...
Prefilter* PrefilterTree::CanonicalNode(Prefilter* node) {
  ...
  std::map<std::string, Prefilter*>::iterator iter = node_map_.find(node_string);
...
void PrefilterTree::AssignUniqueIds(std::vector<std::string>* atom_vec) {
  atom_vec->clear();
...
void PrefilterTree::PrintDebugInfo() {
  ...
  VLOG(10) << "Map:";
  for (std::map<std::string, Prefilter*>::const_iterator iter = node_map_.begin();
