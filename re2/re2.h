...
//   "\\b(\\w+)\\b"        -- \b matches non-empty std::string at word boundary
...
//
// NOTE(rsc): Asking for substd::strings slows successful matches quite a bit.
// This may get a little faster in the future, but right now is slower
// than PCRE.  On the other hand, failed matches run *very* fast (faster
// than PCRE), as do matches without substd::string extraction.
//
// -----------------------------------------------------------------------
// PARTIAL MATCHES
//
// You can use the "PartialMatch" operation when you want the pattern
// to match any substd::string of the text.
//
...
//
// RE2 makes it easy to use any std::string as a regular expression, without
...
// which represents a sub-range of a real std::string.
//
...
//      }
//
// Each successful call to "Consume" will std::vector "var/value", and also
// advance "input" so it points past the matched text.  Note that if the
// regular expression matches an empty std::string, input will advance
// by 0 bytes.  If the regular expression being used might match
// an empty std::string, the loop body must check for this case and either
// advance the std::string or break out of the loop.
//
...
//   // ... std::vector n to the number of RE2::Arg objects ...
...
class RE2 {
 public:
  ...
  // the Option constructor, change the std::vectortings, and pass that
  ...
  // conversions when passing std::string literals to FullMatch and PartialMatch.
  ...
  // The std::string specification for this RE2.  E.g.
  //   RE2 re("ab*c?d+");
  //   re.pattern();    // "ab*c?d+"
  const std::string& pattern() const { return pattern_; }

  // If RE2 could not be created properly, returns an error std::string.
  ...
  // You can pass in a "const char*" or a "std::string" or a "RE2" for "pattern".
  //
  ...
  //      std::string captured as the "i"th sub-pattern.  If you pass in
  //      NULL for the "i"th argument, or pass fewer arguments than
  //      number of sub-patterns, "i"th captured sub-pattern is
  //      ignored.
  //
  // CAVEAT: An optional sub-pattern that does not exist in the
  // matched std::string is assigned the empty std::string.  Therefore, the
  // following will return false (because the empty std::string is not a
  // valid number):
  //    int number;
  ...
  // std::string.  That is, "pattern" need not start its match at the beginning of
  ...
  // of the pattern in the std::string with the rewrite. E.g.
  //
  //   std::string s = "yabba dabba doo";
  ...
  // 'unquoted'.  The returned std::string, used as a regular expression,
  // will exactly match the original std::string.  For example,
  //           1.5-2.0?
  // may become:
  //           1\.5\-2\.0\?
  static std::string QuoteMeta(const StringPiece& unquoted);

  // Computes range for any std::strings matching regexp. The min and max can in
  // some cases be arbitrarily precise, so the caller gets to specify the
  // maximum desired length of std::string returned.
  //
  // Assuming PossibleMatchRange(&min, &max, N) returns successfully, any
  // std::string s that is an anchored match for this regexp satisfies
  //   min <= s && s <= max.
  //
  ...
  // std::vectorting match[0] = "barbaz", match[1] = NULL, match[2] = "bar",
  ...
  // empty std::string, but note that on return, it will not be possible to tell
  // whether submatch i matched the empty std::string or did not match:
  ...
  // Check that the given rewrite std::string is suitable for use with this
  // regular expression.  It checks that:
  //   * The regular expression has enough parenthesized subexpressions
  //     to satisfy all of the \N tokens in rewrite
  //   * The rewrite std::string doesn't have any syntax errors.  E.g.,
  //     '\' followed by anything other than a digit or '\'.
  // A true return value guarantees that Replace() and Extract() won't
  // fail because of a bad rewrite std::string.
  ...
  // Append the "rewrite" std::string, with backslash subsitutions from "vec",
  // to std::string "out".
  // Returns true on success.  This method can fail because of a malformed
  // rewrite std::string.  CheckRewriteString guarantees that the rewrite will
  // be sucessful.
  ...
  class Options {
   public:
    ...
    //   literal          (false) interpret std::string as literal, not regexp
    ...
    void setposix_syntax(bool b) { posix_syntax_ = b; }

    bool longest_match() const { return longest_match_; }
    void setlongest_match(bool b) { longest_match_ = b; }

    bool log_errors() const { return log_errors_; }
    void setlog_errors(bool b) { log_errors_ = b; }

    int max_mem() const { return max_mem_; }
    void setmax_mem(int m) { max_mem_ = m; }

    bool literal() const { return literal_; }
    void setliteral(bool b) { literal_ = b; }

    bool never_nl() const { return never_nl_; }
    void setnever_nl(bool b) { never_nl_ = b; }

    bool never_capture() const { return never_capture_; }
    void setnever_capture(bool b) { never_capture_ = b; }

    bool case_sensitive() const { return case_sensitive_; }
    void setcase_sensitive(bool b) { case_sensitive_ = b; }

    bool perl_classes() const { return perl_classes_; }
    void setperl_classes(bool b) { perl_classes_ = b; }

    bool word_boundary() const { return word_boundary_; }
    void setword_boundary(bool b) { word_boundary_ = b; }

    bool one_line() const { return one_line_; }
    void setone_line(bool b) { one_line_ = b; }
  ...
  // Returns the options std::vector in the constructor.
 ...
 private:
  ...
  std::string                   pattern_;       // std::string regular expression
