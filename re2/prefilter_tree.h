...
// The PrefilterTree class is used to form an AND-OR tree of std::strings
...
// the unique std::strings across the prefilters. During search, by using
// matches from a std::string matching engine, PrefilterTree deduces the
// std::vector of regexps that are to be triggered. The 'std::string matching
// engine' itself is outside of this class, and the caller can use any
// favorite engine. PrefilterTree provides a std::vector of std::strings (called
// atoms) that the user of this class should use to do the std::string
// matching.
//
...
class PrefilterTree {
 public:
  ...
  // The Compile returns a std::vector of std::string in atom_vec.
  // Call this after all the prefilters are added through Add.
  // No calls to Add after Compile are allowed.
  // The caller should use the returned std::vector of std::strings to do std::string matching.
  // Each time a std::string matches, the corresponding index then has to be
  ...
  // contain all the ids of std::string atoms that were found to match the
  // content. The caller can use any std::string match engine to perform
 ...
 private:
  ...
  // A std::string that uniquely identifies the node. Assumes that the
  // children of node has already been assigned unique ids.
  std::string NodeString(Prefilter* node) const;

  // Recursively constructs a readable prefilter std::string.
  ...
  // Map node std::string to canonical Prefilter node.
  ...
  // std::vector of Prefilter for all regexps.
  std::vector<Prefilter*> prefilter_vec_;

  // Atom index in returned std::strings to entry id mapping.
