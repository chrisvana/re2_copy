...
// std::string version of the regular expression and hand it to the
...
// Keep in sync with std::string list kOpcodeNames[] in testing/dump.cc
enum RegexpOp {
  // Matches no std::strings.
  kRegexpNoMatch = 1,

  // Matches empty std::string.
  kRegexpEmptyMatch,
  ...
  // Matches empty std::string at beginning of line.
  kRegexpBeginLine,
  // Matches empty std::string at end of line.
  kRegexpEndLine,
  ...
  // Matches empty std::string at beginning of text.
  kRegexpBeginText,
  // Matches empty std::string at end of text.
  kRegexpEndText,
...
// Keep in sync with std::string list in regexp.cc
...
// This lets us look in a std::vector to find the range covering a particular Rune.
...
class Regexp {
 public:
  ...
  enum ParseFlags {
    ...
    Literal      = 1<<1,   // Treat s as literal std::string instead of a regexp.
  ...
  // Get.  No std::vector, Regexps are logically immutable once created.
  ...
  // Parses std::string s to produce regular expression, returned.
  // Caller must release return value with re->Decref().
  // On failure, std::vectors *status (if status != NULL) and returns NULL.
  ...
  // Parses the regexp src and then simplifies it and std::vectors *dst to the
  // std::string representation of the simplified form.  Returns true on success.
  // Returns false and std::vectors *status (if status != NULL) on parse error.
  ...
  // Returns a std::string representation of the current regexp,
  ...
  // Debugging function.  Returns std::string format for regexp
  ...
  // begin with a non-empty fixed std::string (perhaps after ASCII
  // case-folding).  If so, returns the prefix and the sub-regexp that
  // follows it.
  bool RequiredPrefix(std::string* prefix, bool *foldcase, Regexp** suffix);

 private:
  // Constructor allocates std::vectors as appropriate for operator.
  ...
  // Returns the leading std::string that re starts with.
  ...
  // Simplifies an alternation of literal std::strings by factoring out
  // common prefixes.
  ...
  union {
    ...
    void *the_union_[2];  // as big as any other element, for memstd::vector
  };

  DISALLOW_EVIL_CONSTRUCTORS(Regexp);
};

// Character class std::vector: contains non-overlapping, non-abutting RuneRanges.
