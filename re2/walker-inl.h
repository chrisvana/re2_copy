...
template<typename T> class Regexp::Walker {
 public:
  ...
  // If PreVisit std::vectors *stop to true, the walk does not recurse
  ...
  // The child_args is a std::vector of the T that the child PostVisits returned.
  // PostVisit takes ownership of pre_arg.
  // PostVisit takes ownership of the Ts
  // in *child_args, but not the std::vector itself.
