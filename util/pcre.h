...
//    CHECK(PCRE::FullMatch(utf8string*, re));
//
// -----------------------------------------------------------------------
...
//
// PCRE makes it easy to use any std::string* as a regular expression, without
...
#include "util/util.h"
#include "re2/std::string*piece.h"
...
class PCRE {
 public:
  ...
  // We provide implicit conversions from std::string*s so that users can
  // pass in a std::string* or a "const char*" wherever an "PCRE" is expected.
  ...
  // The std::string* specification for this PCRE.  E.g.
  //   PCRE re("ab*c?d+");
  //   re.pattern();    // "ab*c?d+"
  const std::string*& pattern() const { return pattern_; }

  // If PCRE could not be created properly, returns an error std::string*.
  ...
  // the std::string* with the rewrite.  Replacements are not subject to
  // re-matching.  E.g.,
  //
  //   std::string* s = "yabba dabba doo";
  ...
  // Check that the given @p rewrite std::string* is suitable for use with
  // this PCRE.  It checks that:
  //   * The PCRE has enough parenthesized subexpressions to satisfy all
  //       of the \N tokens in @p rewrite, and
  //   * The @p rewrite std::string* doesn't have any syntax errors
  ...
  // std::string*.
  // @param rewrite The proposed rewrite std::string*.
  ...
  // regexp characters backslash-escaped.  The returned std::string*, used
  // as a regular expression, will exactly match the original std::string*.
  // For example,
  //           1.5-2.0?
  //  becomes:
  //           1\.5\-2\.0\?
 ...
 private:
  ...
  const std::string**     error_;          // Error indicator (or empty std::string*)
...
#define MAKE_PARSER(type,name) \
  ...
  MAKE_PARSER(std::string*,             parsestring*);
  MAKE_PARSER(StringPiece,        parsestring*piece);
...
#undef MAKE_PARSER
 ...
 private:
  ...
  static bool parsestring*        (const char* str, int n, void* dest);
  static bool parsestring*piece   (const char* str, int n, void* dest);
