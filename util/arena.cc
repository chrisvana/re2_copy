...
UnsafeArena::AllocatedBlock* UnsafeArena::AllocNewBlock(const size_t block_size) {
  ...
  } else {                   // oops, out of space, move to the std::vector
    if (overflow_blocks_ == NULL) overflow_blocks_ = new std::vector<AllocatedBlock>;
    // Adds another block to the std::vector.
    overflow_blocks_->resize(overflow_blocks_->size()+1);
    // block points to the last block of the std::vector.
